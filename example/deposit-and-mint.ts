...
async function depositAndMint() {
  ...
  try {
    ...
    const result = await sdk.depositAndMint({
      ...
      positionId,
      depositTokenAddress: tokens.wstETH, // Deposit token address (stETH, weth, or wstETH for ETH; WBTC for BTC)
      depositAmount: parseEther('1'), // Deposit amount (1 ETH)
      mintAmount: parseEther('2000'), // Amount of fxUSD to mint (2000 fxUSD)
      userAddress,
    })
