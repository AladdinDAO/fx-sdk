...
- `example/lock-increase.ts` (build increaseLockAmount txs; set LOCK_AMOUNT_FXN in .env)
- `example/lock-withdraw.ts` (build withdrawLock txs; only when lockStatus is `expired`)
- `example/lock-claim-rewards.ts` (build claimLockRewards txs; shows pendingWstETH before claiming)
